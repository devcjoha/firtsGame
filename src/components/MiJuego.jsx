  ...
  useEffect(() => {
    ...
    ************************************* */
    const bucleJuego = () => {
      ...
      // 2. 🧱 DETECTOR DE COLISIONES AVANZADO (Versión Estable)
      // Pequeña tolerancia para evitar rebotes por bordes finos
      const EPS = 2; // px
      obstaculos.current.forEach((bloque) => {
        // Comprobamos la colisión general (en coordenadas del mundo)
        const chocando =
        ...
        if (chocando) {
          // Calculamos los traslapes en cada eje
          ...
          // Resolvemos por el eje con menor traslape (añadiendo tolerancia)
          if (traslapeX + EPS < traslapeY) {
            // COLISIÓN HORIZONTAL: empujar fuera por X
            if (gamer.vx > 0) {
            ...
            // COLISIÓN VERTICAL: aterrizaje o cabezazo
